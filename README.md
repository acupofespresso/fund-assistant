...
### Phase 2: 增强分析 (即将到来)
- [ ] **多基金对比 (`compare`)**：横向对比 2-4 只基金的收益率、回撤等核心指标。
- [ ] **基金详情 (`info`)**：查询基金规模、资产配置比例（股/债/现）、费率详情。
- [ ] **评级查询 (`rating`)**：集成晨星（Morningstar）等主流机构的风险评级。

### Phase 3: 深度研判 (规划中)
- [ ] **基金经理分析 (`manager`)**：查询经理从业年限、历史任职回报、投资风格标签。
- [ ] **重仓分析 (`holding`)**：穿透查询基金前十大重仓股票及所在行业分布。
